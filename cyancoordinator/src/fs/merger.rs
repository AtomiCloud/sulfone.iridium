...
//         // Collect paths from all three filesystems
//         let mut all_paths = HashSet::new();

//         // Add paths from all three filesystems
//         for path in base.get_paths() {
//             all_paths.insert(path);
//         }

//         for path in current.get_paths() {
//             all_paths.insert(path);
//         }

//         for path in incoming.get_paths() {
//             all_paths.insert(path);
//         }

//         // Empty vector for when content is missing
//         let empty_vec = Vec::new();

//         // Process all unique paths
//         for path in all_paths {
//             // Get content from each filesystem (empty vector if file doesn't exist)
//             let base_content = base.get_file(&path).unwrap_or(&empty_vec);
//             let current_content = current.get_file(&path).unwrap_or(&empty_vec);
//             let incoming_content = incoming.get_file(&path).unwrap_or(&empty_vec);

//             // Log debug information if enabled
//             if self.debug {
//                 println!("--- MERGE DEBUG: {} ---", path.display());

//                 // Show file contents (or indicate empty)
//                 println!("BASE CONTENT: {} bytes", base_content.len());
//                 if !base_content.is_empty() {
//                     println!("{}", String::from_utf8_lossy(base_content));
//                 }

//                 println!("CURRENT CONTENT: {} bytes", current_content.len());
//                 if !current_content.is_empty() {
//                     println!("{}", String::from_utf8_lossy(current_content));
//                 }

//                 println!("INCOMING  : {} bytes", incoming_content.len());
//                 if !incoming_content.is_empty() {
//                     println!("{}", String::from_utf8_lossy(incoming_content));
//                 }
//             }
...
//             // Always perform 3-way merge, treating missing files as empty
//             let merged_result = merge(&base_str, &current_str, &incoming_str);

//             match merged_result {
//                 Ok(merged) => {
//                     // Only add non-empty files to the result
//                     if !merged.is_empty() {
//                         if self.debug {
//                             println!("MERGED CONTENT:");
//                             println!("{}", merged);
//                             println!("--- END MERGE DEBUG: {} ---\n", path.display());
//                         }
//                         result.add_file(path.clone(), merged.into_bytes());
//                     } else if self.debug {
//                         println!("MERGED CONTENT IS EMPTY (file deleted or never existed)");
//                         println!("--- END MERGE DEBUG: {} ---\n", path.display());
//                     }
//                 }
//                 Err(conflict_content) => {
//                     // The diffy merge error contains properly formatted conflict markers
//                     if self.debug {
//                         println!("CONFLICT CONTENT:");
//                         println!("{}", conflict_content);
//                         println!("--- END MERGE DEBUG: {} ---\n", path.display());
//                     }
//                     result.add_file(path.clone(), conflict_content.into_bytes());

//                     // Log the merge conflict to tracing
//                     tracing::warn!("Merge conflict for {}", path.display());
//                 }
//             }
//         }
