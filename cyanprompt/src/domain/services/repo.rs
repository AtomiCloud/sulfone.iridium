...
use crate::http::extension::mapper::{
    extension_ans_input_mapper, extension_ans_output_mapper, extension_validate_input_mapper,
};
...
pub trait CyanRepo {
    ...
    fn prompt_extension(
        &self,
        input: ExtensionAnswerInput,
    ) -> Result<ExtensionOutput, Box<dyn Error + Send>>;

    fn validate_extension(
        &self,
        input: ExtensionValidateInput,
    ) -> Result<Option<String>, Box<dyn Error + 'static + Send + Sync>>;
}
...
impl CyanRepo for CyanHttpRepo {
    ...
    fn validate_extension(
        ...
        self.client.validate_extension(&req).map(|r| r.valid)
    }
}
