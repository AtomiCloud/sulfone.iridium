...
impl CyanClient {
    ...
    pub fn prompt_extension(
        &self,
        r: &ExtensionAnswerReq,
    ) -> Result<ExtensionRes, Box<dyn Error + Send>> {
        let host = (&self.endpoint).to_string().to_owned();
    ...
    pub fn validate_extension(
        &self,
        r: &ExtensionValidateReq,
    ) -> Result<ExtensionValidRes, Box<dyn Error + 'static + Send + Sync>> {
        let host = (&self.endpoint).to_string().to_owned();
