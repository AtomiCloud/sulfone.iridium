...
use crate::domain::models::question::{CheckboxQuestion, ConfirmQuestion, DateQuestion, PasswordQuestion, Question, SelectQuestion, TextQuestion};
use crate::http::core::answer_req::{AnswerReq, BoolAnswerReq, StringAnswerReq, StringArrayAnswerReq};
...
pub fn ans_req_mapper(a: &Answer) -> AnswerReq {
    match a {
        Answer::String(s) => AnswerReq::String(StringAnswerReq {
            answer: s.clone(),
        }),
        Answer::StringArray(sa) => AnswerReq::StringArray(StringArrayAnswerReq {
            answer: sa.clone(),
        }),
        Answer::Bool(b) => AnswerReq::Bool(BoolAnswerReq {
            answer: b.clone(),
        })
    }
}

pub fn prompt_mapper<'a>(q: &'a Question) -> Result<Prompts<'a>, Box<dyn std::error::Error + Send>> {
    match q {
        Question::Confirm(c) => Ok(inquire::Confirm::new(&c.message))
            .map(|p| c.default.map_or(p.clone(), |def| p.with_default(def)))
            .map(|p| c.desc.as_ref().map_or(p.clone(), |help| p.with_help_message(help)))
            .map(|p| c.error_message.as_ref().map_or(p.clone(), |err_msg| p.with_error_message(err_msg)))
            .map(|p| Prompts::Confirm(p)),
        Question::Date(date) => Ok(DateSelect::new(&date.message))
            .map(|p| date.desc.as_ref().map_or(p.clone(), |desc| p.with_help_message(desc)))
            .and_then(|p| date.default.as_ref().map_or(Ok(p.clone()), |s| NaiveDate::parse_from_str(s, "%Y-%m-%d").map(|e| p.with_default(e)).map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send>)))
            .and_then(|p| date.min_date.as_ref().map_or(Ok(p.clone()), |s| NaiveDate::parse_from_str(s, "%Y-%m-%d").map(|e| p.with_min_date(e)).map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send>)))
            .and_then(|p| date.max_date.as_ref().map_or(Ok(p.clone()), |s| NaiveDate::parse_from_str(s, "%Y-%m-%d").map(|e| p.with_max_date(e)).map_err(|e| Box::new(e) as Box<dyn std::error::Error + Send>)))
            .map(|p| Prompts::Date(p)),
        Question::Checkbox(cb) => Ok(MultiSelect::new(&cb.message, cb.options.clone()))
            .map(|p| cb.desc.as_ref().map_or(p.clone(), |desc| p.with_help_message(desc)))
            .map(|p| Prompts::Checkbox(p)),
        Question::Password(pw) => Ok(inquire::Password::new(&pw.message)
            .with_display_mode(PasswordDisplayMode::Masked))
            .map(|p| pw.desc.as_ref().map_or(p.clone(), |desc| p.with_help_message(desc)))
            .map(|p| pw.confirmation.map_or(p.clone(), |confirm| if confirm { p.clone() } else { p.without_confirmation() }))
            .map(|p| Prompts::Password(p)),
        Question::Text(text) => Ok(inquire::Text::new(&text.message))
            .map(|p| text.desc.as_ref().map_or(p.clone(), |desc| p.with_help_message(desc)))
            .map(|p| text.default.as_ref().map_or(p.clone(), |def| p.with_default(def)))
            .map(|p| text.initial.as_ref().map_or(p.clone(), |init| p.with_initial_value(init)))
            .map(|p| Prompts::Text(p)),
        Question::Select(s) => Ok(inquire::Select::new(&s.message, s.options.clone()))
            .map(|p| s.desc.as_ref().map_or(p.clone(), |desc| p.with_help_message(desc)))
...
pub fn cyan_res_mapper(r: CyanRes) -> Cyan {
    Cyan {
        processors: r.processors.iter().map(|x| processor_res_mapper(x.clone())).collect(),
        plugins: r.plugins.iter().map(|x| plugin_res_mapper(x.clone())).collect(),
    }
}
...
pub fn cyan_req_mapper(r: Cyan) -> CyanReq {
    CyanReq {
        processors: r.processors.iter().map(|x| processor_req_mapper(x.clone())).collect(),
        plugins: r.plugins.iter().map(|x| plugin_req_mapper(x.clone())).collect(),
    }
}