...
use crate::state::traits::StateManager;
...
impl DefaultStateManager {
    ...
    fn load_or_create_state_file(&self, path: &Path) -> Result<CyanState, Box<dyn Error + Send>> {
        if path.exists() {
...
impl StateManager for DefaultStateManager {
    fn save_template_metadata(
        ...
        let mut state = self.load_or_create_state_file(&state_file_path)?;
