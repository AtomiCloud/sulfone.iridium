...
impl fmt::Display for ParsingError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ParsingError::FailedParsingPluginReference(s) => write!(f, "Incorrect Plugin Reference: {}", s),
            ParsingError::FailedParsingProcessorReference(s) => write!(f, "Incorrect Processor Reference: {}", s)
        }
    }
}

pub fn read_file(config_path: String) -> Result<String, Box<dyn Error+ Send>> {
    let resp: Result<String, Box<dyn Error+ Send>> = fs::read_to_string(config_path)
...
pub fn template_config_mapper(r: &CyanTemplateFileConfig) -> Result<CyanTemplateConfig, Box<dyn Error+ Send>> {
    let proc: Result<Vec<CyanProcessorRef>, Box<dyn Error+ Send>> = r
        .processors
        .iter()
        .map(|p| processor_reference_mapper(p.clone()))
        .map(|opt| opt
            .ok_or(ParsingError::FailedParsingProcessorReference("unknown".to_string()))
            .map_err(|e| Box::new(e) as Box<dyn Error+ Send>)
        )
        .collect();

    let plug: Result<Vec<CyanPluginRef>, Box<dyn Error+ Send>> = r.plugins
        .iter()
    ...
    let readme_result: Result<String, Box<dyn Error+ Send>> = fs::read_to_string(r.readme.clone())
        .map_err(|e| Box::new(e) as Box<dyn Error+ Send>);

    let combined_result = proc
...
pub fn processor_config_mapper(r: &CyanProcessorFileConfig) -> Result<CyanProcessorConfig, Box<dyn Error+ Send>> {
...
pub fn plugin_config_mapper(r: &CyanPluginFileConfig) -> Result<CyanPluginConfig, Box<dyn Error+ Send>> {
