...
use crate::cli::mapper::{plugin_config_mapper, processor_config_mapper, read_yaml, template_config_mapper};
...
impl CyanRegistryClient {
    fn json_post<T: Serialize + ?Sized>(&self, endpoint: String, r: &T, token: Option<String>) -> Result<Response, reqwest::Error> {
    ...
    fn push_processor_internal(&self, username: String, token: String, r: &ProcessorReq) -> Result<ProcessorVersionPrincipalRes, Box<dyn Error + Send>> {
        let host = (&self.endpoint).to_string().to_owned();
        let version = (&self.version).to_string().to_owned();
        let endpoint = host + "/api/v".to_string().as_str() + version.as_str() + "/Processor/push/".to_string().as_str() + username.as_str();
        ...
        self.json_post(endpoint, r, Some(token))
            ...
            .and_then(|x| {
                if x.status().is_success() {
                    ...
                    let r: Result<ProblemDetails, Box<dyn Error + Send>> = x.json().map_err(|e| Box::new(e) as Box<dyn Error + Send>);
                    match r {
    ...
    fn push_plugin_internal(&self, username: String, token: String, r: &PluginReq) -> Result<PluginVersionPrincipalRes, Box<dyn Error + Send>> {
        let host = (&self.endpoint).to_string().to_owned();
        let version = (&self.version).to_string().to_owned();
        let endpoint = host + "/api/v".to_string().as_str() + version.as_str() + "/Plugin/push/".to_string().as_str() + username.as_str();
    ...
    fn push_template_internal(&self, username: String, token: String, r: &TemplateReq) -> Result<TemplateVersionPrincipalRes, Box<dyn Error + Send>> {
        let host = (&self.endpoint).to_string().to_owned();
        let version = (&self.version).to_string().to_owned();
        let endpoint = host + "/api/v".to_string().as_str() + version.as_str() + "/Template/push/".to_string().as_str() + username.as_str();
    ...
    pub fn get_template(&self, username: String, name: String, v: Option<i64>) -> Result<TemplateVersionRes, Box<dyn Error + Send>> {
        let host = (&self.endpoint).to_string().to_owned();
        let version = (&self.version).to_string().to_owned();

        let endpoint = match v {
            None => format!("{}/api/v{}/Template/slug/{}/{}/versions/latest?bumpDownload=true", host, version, username, name),
            Some(ver) => format!("{}/api/v{}/Template/slug/{}/{}/versions/{}?bumpDownload=true", host, version, username, name, ver)
        };
        self.client
            .get(endpoint)
            .send()
    ...
    pub fn get_plugin(&self, username: String, name: String, v: Option<i64>) -> Result<PluginVersionRes, Box<dyn Error + Send>> {
        let host = (&self.endpoint).to_string().to_owned();
        let version = (&self.version).to_string().to_owned();

        let endpoint = match v {
            None => format!("{}/api/v{}/Plugin/slug/{}/{}/versions/latest?bumpDownload=true", host, version, username, name),
            Some(ver) => format!("{}/api/v{}/Plugin/slug/{}/{}/versions/{}?bumpDownload=true", host, version, username, name, ver)
        };
    ...
    pub fn get_processor(&self, username: String, name: String, v: Option<i64>) -> Result<ProcessorVersionRes, Box<dyn Error + Send>> {
        let host = (&self.endpoint).to_string().to_owned();
        let version = (&self.version).to_string().to_owned();

        let endpoint = match v {
            None => format!("{}/api/v{}/Processor/slug/{}/{}/versions/latest?bumpDownload=true", host, version, username, name),
            Some(ver) => format!("{}/api/v{}/Processor/slug/{}/{}/versions/{}?bumpDownload=true", host, version, username, name, ver)
        };
        self.client
            .get(endpoint)
            .send()
